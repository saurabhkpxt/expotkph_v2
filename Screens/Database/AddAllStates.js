...
export default () => {
  ...
  function resetDB() {
    ...
    db.transaction(
      (tx) => {
        tx.executeSql("drop table if exists allstates1;");
        tx.executeSql(
  ...
  function fetchDB() {
    ...
    db.transaction(
      (tx) => {
        tx.executeSql(
          "create table if not exists allstates1 (id TEXT primary key not null, state TEXT, district TEXT);"
        );
